...
export async function PATCH(
  ...
  try {
    ...
    const { descripcion, monto, categoria, metodoPago, notas, fecha, insumo } =
      body;
    ...
    if (categoria === "INSUMOS" && insumo) {
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (insumoExistente) {
          const cantidadUsadaEnProductos =
            Number(insumoExistente.cantidadTotal) -
            Number(insumoExistente.cantidadDisponible);
          const nuevoCantidadDisponible = Math.max(
            0,
            insumo.cantidadTotal - cantidadUsadaEnProductos,
          );
        ...
        const insumoActualizado = await tx.insumo.findFirst({
          where: { gastoId: id },
        });
          ...
          insumo: insumoActualizado
            ? {
                ...
                cantidadDisponible: Number(
                  insumoActualizado.cantidadDisponible,
                ),
...
export async function DELETE(
  ...
  try {
    ...
    const gasto = await prisma.gasto.findUnique({
      ...
      include: {
        insumos: {
          select: { id: true, cantidadTotal: true, cantidadDisponible: true },
        },
      },
    });
    if (!gasto) {
    ...
    const insumo = gasto.insumos[0];
    if (insumo) {
      if (Number(insumo.cantidadDisponible) < Number(insumo.cantidadTotal)) {
        return NextResponse.json<RespuestaAPI<null>>(
          {
            error:
              "No se puede eliminar el gasto porque el insumo asociado ya se está utilizando en productos. Primero eliminar/modifcar productos",
          },
          { status: 400 },
        );
      }
      await prisma.$transaction([
        prisma.insumo.delete({ where: { id: insumo.id } }),
